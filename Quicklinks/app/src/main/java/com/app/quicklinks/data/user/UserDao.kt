...
interface UserDao {
    ...
    @Query("SELECT id FROM users WHERE email = :emailOrUsername AND password = :password OR username = :emailOrUsername AND password = :password LIMIT 1")
    suspend fun getID(emailOrUsername: String, password: String): Long?
